Helpful Sources for the project:

1. Python CSV Files: Reading and Writing:
https://dzone.com/articles/python-101-reading-and-writing

2. pprint : Data pretty printer in Python:
https://www.geeksforgeeks.org/pprint-data-pretty-printer-python/

3. CSV Writer Objects: Python Documentation:
https://docs.python.org/3/library/csv.html#csv.writer

4. How To Plot Data in Python 3 Using matplotlib:
https://www.digitalocean.com/community/tutorials/how-to-plot-data-in-python-3-using-matplotlib

5. Matplotlib Tutorial: Python Plotting:
https://www.datacamp.com/community/tutorials/matplotlib-tutorial-python

6. Python Dict and File:
https://developers.google.com/edu/python/dict-files

7. Python Dictionary:
https://www.programiz.com/python-programming/dictionary

8. How to Think Like a Computer Scientist: Learning with Python 3: Dictionaries:
http://openbookproject.net/thinkcs/python/english3e/dictionaries.html

9. Python 3 - Dictionary:
https://www.tutorialspoint.com/python3/python_dictionary.htm

10. Iterating Over Dictionary in Python:
http://dev-notes.eu/2017/09/iterating-over-dictionary-in-python/

11. Understanding Dictionaries in Python 3:
https://www.digitalocean.com/community/tutorials/understanding-dictionaries-in-python-3

Other than these links, I have taken a lot of help from stackoverflow.com.
